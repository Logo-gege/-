      ...
      <div className="bg-black/40 backdrop-blur-3xl border border-white/10 border-l-[3px] border-l-[#2ecc71] rounded-r-3xl py-10 px-4 flex flex-col items-center gap-10 shadow-2xl w-[140px] relative overflow-hidden transition-all duration-500">
        <div className="flex flex-col items-center gap-1 min-h-[120px] justify-center">
          {getStatusTitle().split('').map((char, i) => (
            <span key={i} className="text-4xl font-xianshi font-bold text-[#2ecc71] leading-none drop-shadow-[0_0_15px_#2ecc71] transition-all duration-500 animate-in fade-in slide-in-from-top-2">
              {char}
            </span>
          ))}
        </div>
        ...
        <div className="flex flex-col gap-6 items-center text-center">
          ...
          <div className="flex flex-col gap-1">
            <p className="text-[9px] text-white/30 tracking-widest uppercase mb-1 font-xianshi">境界</p>
            <p className={`text-xs font-bold tracking-tighter transition-colors duration-500 font-xianshi ${displayProgress >= 100 ? 'text-yellow-400' : 'text-[#2ecc71]'}`}>
              {displayProgress >= 100 ? '灵气溢满' : '积攒修为'}
            </p>
          </div>
        </div>
        <div className="flex flex-col items-center gap-3 relative">
          ...
          <div className="w-2 h-40 bg-white/5 rounded-full overflow-hidden relative border border-white/10 p-[1px]">
            <div 
              className={`absolute bottom-0 left-0 w-full transition-all duration-300 ${displayProgress >= 100 ? 'bg-yellow-400' : 'bg-[#2ecc71]'} ${displayProgress >= 100 ? 'animate-pulse' : ''}`} 
              style={{ height: `${displayProgress}%`, boxShadow: `0 0 10px ${displayProgress >= 100 ? '#f1c40f' : '#2ecc71'}` }} 
            />
          </div>
...
const App: React.FC = () => {
  ...
  const handleMonsterKill = useCallback(() => {
    setKills(prev => {
      ...
      audioService.playMonsterHowl(); // 新增：播放妖兽嚎叫
      setShake(true);
  ...
  useEffect(() => {
    ...
    const update = () => {
      ...
      if (!isTransitioning.current && !isLocked.current && !isRainActive) {
        ...
        const requiredConfirmTime = (rawHandCount === 2) ? 300 : 100;
        if (modeConfirmTimer.current >= requiredConfirmTime) {
        ...
        if (rawHandCount > 0) {
          ...
          if (mode === 'single' || (mode === 'array' && rawHandCount === 1)) {
            ...
            const catchUpPower = Math.min(0.9, 0.25 + (dist * 1.5) + (speedMagnitude * 20));
            const frameFactor = 1 - Math.pow(1 - catchUpPower, dt / 16.6);
            
            if (dist > 3.0) {
              stableTargetPos.current.copy(predictedPos);
            } else {
              stableTargetPos.current.lerp(predictedPos, frameFactor);
            }
            
            swordTargetRef.current.copy(stableTargetPos.current);
            lastRawPos.current.copy(rawPos);
            
            audioService.updateSwordSFX(speedMagnitude * 120, dist);
            lateralVelocity.current = THREE.MathUtils.lerp(lateralVelocity.current, velocity.current.x, 0.3);
            
            const targetRotationZ = Math.atan2(-(tip.y - mcp.y), -(tip.x - mcp.x)) - Math.PI / 2;
            const targetRotationX = THREE.MathUtils.clamp(lateralVelocity.current * 0.15, -0.6, 0.6);
            
            stableRotation.current.x = THREE.MathUtils.lerp(stableRotation.current.x, targetRotationX, 0.2);
            stableRotation.current.z = THREE.MathUtils.lerp(stableRotation.current.z, targetRotationZ, 0.25);
            
            swordRotationRef.current.x = stableRotation.current.x;
          ...
          audioService.updateSwordSFX(0, 0);
          if (mode !== 'array') setHint('搜寻妖气');
        }
      }
  ...
  return (
    <div className={`relative w-full h-screen bg-black overflow-hidden transition-all duration-75 ${shake ? 'scale-[1.02] grayscale-[0.2]' : 'scale-100'}`}>
      <Scene 
      ...
      {!isCameraActive && (
        <div className="absolute inset-0 flex flex-col items-center justify-center bg-[#05080a] z-[100] overflow-hidden">
           <div className="absolute inset-0 ink-cloud bg-[#2ecc71]/5" />
           <div className="relative group cursor-pointer flex items-center justify-center w-full h-full max-w-[900px] max-h-[900px]" onClick={startCamera}>
              <div className="absolute w-[500px] h-[500px] opacity-10 animate-pulse-soft pointer-events-none">
                 <svg viewBox="0 0 100 100" className="w-full h-full fill-[#2ecc71]">
                    ...
                    <path d="M50 10c-22.1 0-40 17.9-40 40s17.9 40 40 40V10z" opacity="0.5"/>
                    <circle cx="50" cy="30" r="5" />
                    <circle cx="50" cy="70" r="5" fill="black" />
                 </svg>
              </div>
              <div className="absolute w-[650px] h-[650px] animate-spin-slow flex items-center justify-center">
                 {trigrams.map((t, i) => (
                    <div key={i} className="bagua-item font-bold" style={{ transform: `rotate(${i * 45}deg) translateY(-280px)` }}>{t}</div>
                 ))}
              </div>
              <div className="absolute w-[420px] h-[420px] rounded-full border-[2px] border-[#2ecc71]/20 border-dashed animate-spin-reverse" />
              <div className="absolute w-[450px] h-[450px] rounded-full border-[1px] border-[#2ecc71]/10 animate-spin-slow" />
              <div className="absolute w-[340px] h-[340px] rounded-full border-[4px] border-[#2ecc71]/40 shadow-[0_0_80px_rgba(46,204,113,0.4)] transition-all duration-700 group-hover:shadow-[0_0_120px_rgba(46,204,113,0.6)] group-hover:scale-105" />
              <div className={`absolute z-50 flex flex-col items-center transition-all duration-1000 ${isStarting ? 'scale-125 opacity-0 blur-3xl' : 'scale-100 opacity-100'}`}>
                 <div className="transform scale-x-[1.1] tracking-[0.2em] font-bold font-calligraphy">
                    <h1 className="text-[52px] text-3d-emboss select-none">凡人修仙</h1>
                 </div>
                 <div className="flex items-center gap-4 mt-6">
                    <div className="h-[1px] w-6 bg-[#2ecc71]/30" />
                    <p className="text-[#2ecc71]/60 text-[9px] tracking-[0.6em] uppercase font-cinzel animate-pulse font-xianshi">Manifest Thy Spirit</p>
                    <div className="h-[1px] w-6 bg-[#2ecc71]/30" />
                 </div>
                 <div className="mt-10 text-[#2ecc71]/40 text-[10px] font-xianshi tracking-[0.3em] opacity-0 group-hover:opacity-100 transition-opacity duration-500">点击屏幕 • 开启仙路</div>
              </div>
           </div>
        </div>
      )}
      ...
      <div className={`absolute top-10 right-10 transition-all duration-1000 ${isCameraActive ? 'opacity-90' : 'opacity-0'}`}>
        <div className="w-48 h-32 bg-black/40 backdrop-blur-xl border border-white/10 rounded-3xl overflow-hidden shadow-2xl relative">
          <video ref={videoRef} className="w-full h-full object-cover grayscale-[0.2] opacity-80 scale-x-[-1]" autoPlay muted playsInline />
          <div className="absolute top-0 left-0 w-full h-[2px] bg-[#2ecc71]/40 animate-scan pointer-events-none" />
        </div>
      </div>
    </div>
  );
